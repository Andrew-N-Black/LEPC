...
ggplot(ROH_habitat, aes(fill=SIZE, y=FROH, x=reorder(HABITAT,FROH))) + geom_bar(stat="identity")+scale_fill_manual("ROH Length", values =c("100KB"="orange","1MB"="black"))+xlab("")+ylab("fROH")+theme(axis.title.x=element_blank(),axis.text.x=element_blank(),axis.ticks.x=element_blank())+theme_classic()+coord_flip()
ggsave("~/habitat_roh.svg",width = 8, height = 2 )
